...
- [x] Email/password registration _(Notes: Successfully implemented email and password registration functionality via `app/signup/page.tsx`. Users can create accounts, which are stored in Supabase `auth.users` table. The signup form allows users to select their role (User/Washer), which is passed as `selected_role` in `options.data` during the `supabase.auth.signUp()` call and stored in `auth.users.raw_user_meta_data`. Basic form validation and error handling are in place. Added optional 'Referral Code' input, storing `submitted_referral_code` in `user_metadata`. **Password input enhanced with visibility toggle and Caps Lock indicator.**)_
...
- [x] Google OAuth integration _(Notes: Paused/Deferred. UI elements removed from signup/signin pages (app/signup/page.tsx, app/signin/page.tsx) due to complexities with Supabase metadata handling (selected_role placement) for new OAuth users and to simplify the initial auth flow. The set-default-role Edge Function logic related to Google users is now dormant. Can be revisited.)_
- [x] User profile creation _(Notes: Profile creation in the `profiles` table is now automated via the `handle_new_user` trigger in Supabase, which fires after a new entry in `auth.users`. This trigger reads `selected_role` from `auth.users.raw_user_meta_data` (set during signup) and populates `profiles.role` and an initial `profiles.washer_status` (e.g., 'pending_application' if role is 'washer'). UI forms and Supabase save/load logic for Profile Information (First Name, Last Name, Phone, Postcode, Email (display-only)), Notification Preferences, and Laundry Preferences are implemented in `app/dashboard/settings/page.tsx`. The page fetches and displays this profile data. Files: `app/dashboard/settings/page.tsx`, Supabase trigger `handle_new_user`.)_
- [ ] Washer profile creation \_(Notes: ...)
- [~] Reviews page _(Notes: Created app/reviews/page.tsx. Implemented Hero section, Overall Rating display, Rating Distribution chart, and a static grid of Featured Review cards with placeholder data. Included verified badges and placeholders for filters and loading more reviews. Resolved routing conflict by deleting app/(landing)/reviews/page.tsx. Files: app/reviews/page.tsx, components/layout/Header.tsx (verified link exists))_
...
- [x] Supabase schema setup _(Notes: `referrals` and `referral_events` tables created. A `profiles` table is also created with columns for `id` (FK to auth.users), `email`, `first_name`, `last_name`, `phone_number`, `postcode`, `notification_preferences` (jsonb), `allergies`, `product_preferences`, `role` (TEXT), `washer_status` (TEXT), `created_at`, `updated_at`. RLS policies for SELECT, INSERT, UPDATE on `profiles` are in place. **A new user trigger (`handle_new_user` on `auth.users` after insert) now automatically populates the `profiles` table, setting the `role` based on `auth.users.raw_user_meta_data->>'selected_role'` (captured at signup) and an initial `washer_status` if the role is 'washer' (e.g., 'pending_application').**) A `washer_interest_registrations` table has also been created with appropriate columns and RLS policies to manage users expressing interest in becoming washers.)_
- [x] API endpoints implementation _(Notes: `process-referral` Supabase Edge Function created. Basic user profile data creation is automated via the `handle_new_user` trigger. Profile editing is managed client-side via Supabase SDK in `app/dashboard/settings/page.tsx`.)_
- [ ] Real-time chat functionality \_(Notes: ...)
