...
export async function registerWasherInterest(userId: string, area: string): Promise<{ success: boolean; error?: string }> {
  if (!userId) {
    return { success: false, error: 'You must be logged in to register interest.' };
  }
  ...
  if (existingInterest) {
    return { success: false, error: 'You have already registered your interest.' }
  }
  ...
  if (insertError) {
    ...
    return { success: false, error: 'Could not save your interest. Please try again.' }
  }
